package com.day6;
/**
  一、异常Exception：1、异常事件两大类 a、Error（Stackoverflow，out of memory） b、Exception：编译异常和运行异常
                ...
                4、自定义异常一般继承RunTimeException（运行时异常），且使用throw抛出异常
                   CustomizeException
  二、包装类：1、继承了Number（long，int，double，short）；boolean；char；
         2、手动拆箱、手动装箱和自动拆箱、自动装箱 Integer_ (可以相互转换两种类型)
  三、String类： 1、String s = new String("") <-先指向的是堆内存在指向常量池  String s = "" <-直接指向的是常量池 其中两个s都是是在栈内存
  ...
  四、日期(第三代)
       ...
       idt.getYear()、idt.getMonthValue()........
  五、注解
       ①@SuppressWarnings({"all"})：去掉所有警告
       ②@Deprecated 修饰某个元素表示该元素已过期
       ③@Override：限定某个方法，重写和实现方法的标志，只能用于方法
